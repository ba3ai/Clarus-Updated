...
import { AuthContext } from "../context/AuthContext"; // ✅ Import AuthContext to detect login

const fundData = {
  name: "General Partner Fund ",
  status: "ACTIVE",
//   date: "June 5th, 2023",
  committedCapital: "$6,000,000",
  calledSoFar: "$100,000",
  fundsAvailable: "$30,000",
  leftToCall: "$90,000",
  investments: [
    { company: "Brex", currentValue: "$1,345,223", initialInvestment: "$1,000,000", lastUpdated: "1/11/23", positive: true },
    { company: "Cardless", currentValue: "$100,000", initialInvestment: "$100,000", lastUpdated: "2/17/23", positive: true },
    { company: "Kaycakes", currentValue: "$432,567", initialInvestment: "$600,000", lastUpdated: "2/14/23", positive: false },
    { company: "Pipe", currentValue: "$710,436", initialInvestment: "$800,000", lastUpdated: "10/02/23", positive: false },
  ],
};
...
const SidebarItem = ({ icon, label, active, onClick }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-3 w-full px-3 py-2 rounded-md text-sm font-medium transition ${active ? "bg-blue-100 text-blue-700" : "text-gray-700 hover:bg-blue-50 hover:text-blue-600"}`}
  >
    {icon}
    {label}
  </button>
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 shadow-sm flex flex-col justify-between">
        <div>
          <div className="p-5 border-b border-gray-100 text-center">
            <h1 className="text-xl font-bold text-gray-800">General Partner</h1>
        ...
        {user && (
          <div className="p-4 border-t border-gray-200 space-y-2">
            <button
              ...
              className="flex items-center gap-2 text-gray-800 hover:text-blue-600 w-full text-md"
            >
      ...
      <main className="flex-1 p-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <span className="text-sm bg-green-100 text-green-700 px-2 py-1 rounded">{fundData.status}</span>
            <h1 className="text-2xl font-bold mt-2">{fundData.name}</h1>
            <p className="text-gray-500">{fundData.date}</p>
          </div>
          {/* <div className="space-x-2">
            <button className="px-4 py-2 bg-gray-900 text-white rounded">Start Investment</button>
                ...
                <tbody>
                  {fundData.investments.map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className={`p-2 border font-medium ${item.positive ? "text-green-600" : "text-red-600"}`}>{item.currentValue}</td>
                      <td className="p-2 border">{item.initialInvestment}</td>
                      <td className="p-2 border">{item.lastUpdated}</td>
                      <td className="p-2 border text-blue-600 cursor-pointer" onClick={() => setShowHistory(true)}>
                        View
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            <div className="flex justify-between mb-4">
              <h3 className="text-lg font-bold">Company History</h3>
              <button onClick={() => setShowHistory(false)} className="text-gray-500 hover:text-gray-700">X</button>
            </div>
            ...
            <div className="text-right mt-4">
              <button onClick={() => setShowHistory(false)} className="bg-gray-900 text-white px-4 py-2 rounded">
                Close
              </button>
            </div>
          </div>
